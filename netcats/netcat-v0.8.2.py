...
help = YELLOW + """\n\t\t-------------- Help ------------------\n
    ...
    target_ip\t\ttarget-ip             \t\tit shows the target ip
...
def ra_pick():
    list1 =["wellcom",
                "we are connected",
                "version = netcut_v0.8.2",
                "you been hacked 😈"
                ]
    u_pic_me = random.choice(list1)
    return u_pic_me
...
while True:
    ...
    try:
        com = input(GREEN+"["+END+BLUE+"●"+END+GREEN+"]["+END+MAGENTA+ra_pick()+END+GREEN+"<==>"+END+YELLOW+"Net-Cat"+END+GREEN+"]"+END+BLUE+"> "+END+GREEN)
        """help me"""+END
        if com[:8]=="download":
